...
function generateListWord(){
  ...
  function wordPreparation(rndVal){
    ...
    /*p2*/   (rndVal.adverb == 0) ? curPronRus2 = bubaboba[rndVal.pron2] : curPronRus2 = bubaboba[rndVal.pron2 + 6];
    /*p*/    curV2E = v2E[0];
    /*p*/    curV2R = v2R[0];
    /*p*/    to = "to";
    /*p2*/   withWithoutRUS = biba[rndVal.adverb];
    /*p2*/   withWithoutENG = buba[rndVal.adverb];
    /*p*/  } else {  
    /*p*/    curVerbEng = listVerbEng[rndVal.verb];
    /*p*/    curVerbRus = listVerbRus[getVerbRus(rndVal.cell, rndVal.pron1, rndVal.verb)];
    /* lvl2 */  curPronRus2 = arrPronRus2[rndVal.pron2];
    /*p*/    curV2E = "";
    /*p*/    curV2R = "";
    /*p*/    to = "";
    /*p2*/    withWithoutRUS = "";
  ...
  function generateString(){
    if ( plus(binaryVerbs) ) {
      let rndVal = blessRNG( {verbList: listVerbEng.length, pronCnt: 2, to: true, adverb: true, verb2: plusE} );
      ...
      console.log("успех!!!!")
        stringQestion = `${curQuestionEng} ${curToDo1} ${curPronEng} ${curToDo2} ${curVerbEng} ${to} ${curV2E} ${withWithoutENG} ${curPronEng2}`;
        stringAnswer = `${curQuestionRus} ${curPronRus} ${curToDoRusN} ${curToDoRusQ} ${curVerbRus}  ${curV2R} ${withWithoutRUS} ${curPronRus2} `;
      } else if (rndVal.order == "rus-eng" || (rndVal.order == "mix" && rndVal.mix == 1)) {
        stringQestion = `${curQuestionRus} ${curPronRus} ${curToDoRusN} ${curToDoRusQ} ${curVerbRus} ${curV2R} ${withWithoutRUS} ${curPronRus2}`;
        stringAnswer = `${curQuestionEng} ${curToDo1} ${curPronEng} ${curToDo2} ${curVerbEng} ${to}  ${curV2E} ${withWithoutENG} ${curPronEng2}`;
      }
